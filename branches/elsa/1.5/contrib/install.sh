...
build_node_perl(){	
	...
	# Specific version of Test::CheckDeps until bug fixed: https://rt.cpan.org/Public/Bug/Display.html?id=89366 
	cpanm Test::CheckDeps@0.007
...
update_node_mysql(){
	...
	mysql -u$MYSQL_ROOT_USER $MYSQL_PASS_SWITCH $MYSQL_NODE_DB -e 'ALTER TABLE imports ADD `first_id` KEY(first_id)' > /dev/null 2>&1
	mysql -u$MYSQL_ROOT_USER $MYSQL_PASS_SWITCH $MYSQL_NODE_DB -e 'ALTER TABLE imports ADD `last_id` KEY(last_id)' > /dev/null 2>&1
